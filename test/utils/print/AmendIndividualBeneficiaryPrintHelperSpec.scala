    ...
    "generate individual beneficiary section for all possible data" in {
      ...
      result mustBe AnswerSection(
        ...
        rows = Seq(
          ...
          AnswerRow(label = Html(messages("individualBeneficiary.shareOfIncome.checkYourAnswersLabel", name.displayName)), answer = Html("50%"), changeUrl = controllers.individualbeneficiary.amend.routes.IncomePercentageController.onPageLoad().url),
