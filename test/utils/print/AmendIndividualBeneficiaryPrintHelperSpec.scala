    ...
    "generate individual beneficiary section for all possible data" in {
      ...
      result mustBe AnswerSection(
        ...
        rows = Seq(
          ...
          AnswerRow(label = Html(messages("individualBeneficiary.nationalInsuranceNumber.checkYourAnswersLabel", name.displayName)), answer = Html("AA 00 00 00 A"), changeUrl = controllers.individualbeneficiary.amend.routes.NationalInsuranceNumberYesNoController.onPageLoad().url),
