...
import pages.other.{AddressUkYesNoPage, DescriptionPage}
import play.api.data.Form
import play.api.inject.bind
import play.api.mvc.Call
import play.api.test.FakeRequest
import play.api.test.Helpers._
import views.html.other.amend.AddressUkYesNoView

class AddressUkYesNoControllerSpec extends SpecBase with MockitoSugar {

  private val form: Form[Boolean] = new YesNoFormProvider().withPrefix("otherBeneficiary.addressUkYesNo")
  ...
  "AddressUkYesNo Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      contentAsString(result) mustEqual
        view(form.fill(true), description)(fakeRequest, messages).toString
    ...
    "redirect to the next page when valid data is submitted" in {
      ...
      val application =
        applicationBuilder(userAnswers = Some(emptyUserAnswers))
          .overrides(
            bind[Navigator].qualifiedWith(classOf[AmendOtherBeneficiary]).toInstance(new FakeNavigator(onwardRoute))
          ).build()
