...
class NonUkAddressControllerSpec extends SpecBase with MockitoSugar {
  ...
  "NonUkAddress Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      contentAsString(result) mustEqual
        view(form.fill(answer), mode, countryOptions, description)(request, messages).toString
    ...
    "return a Bad Request and errors when invalid data is submitted" in {
      ...
      val request = FakeRequest(POST, nonUkAddressRoute).withFormUrlEncodedBody("value" -> "")
