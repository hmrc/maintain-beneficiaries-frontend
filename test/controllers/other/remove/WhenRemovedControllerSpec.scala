...
class WhenRemovedControllerSpec extends SpecBase with MockitoSugar {
  ...
  private def form = formProvider.withPrefixAndEntityStartDate("otherBeneficiary.whenRemoved", LocalDate.now())
  ...
  def otherBeneficiary(id: Int): OtherBeneficiary = OtherBeneficiary(
  ...
  val beneficiaries: List[OtherBeneficiary] = List(otherBeneficiary(1), otherBeneficiary(2), otherBeneficiary(3))
