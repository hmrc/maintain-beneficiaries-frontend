...
import pages.other.RemoveYesNoPage
...
class RemoveOtherBeneficiaryControllerSpec extends SpecBase with ScalaCheckPropertyChecks with ScalaFutures {
  ...
  def otherBeneficiary(id: Int, provisional : Boolean) = OtherBeneficiary(
  ...
  val expectedResult = otherBeneficiary(2, provisional = true)
  ...
  "RemoveOtherBeneficiary Controller" when {
    ...
    "return OK and the correct view for a GET" in {
      ...
      contentAsString(result) mustEqual view(form, index, description)(fakeRequest, messages).toString
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      contentAsString(result) mustEqual
        view(form.fill(true), 0, description)(fakeRequest, messages).toString
