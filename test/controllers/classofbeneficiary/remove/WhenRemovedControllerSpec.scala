...
class WhenRemovedControllerSpec extends SpecBase with MockitoSugar {
  ...
  "WhenRemoved Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      contentAsString(result) mustEqual
        view(form.fill(validAnswer), index, description)(getRequest(), messages).toString
    ...
    "redirect to the next page when valid data is submitted" in {
      ...
      when(mockConnector.removeClassOfBeneficiary(any(), any())(any(), any())).thenReturn(Future.successful(HttpResponse(200)))
