...
class RemoveEmploymentBeneficiaryControllerSpec extends SpecBase with ScalaCheckPropertyChecks with ScalaFutures {

  val messagesPrefix = "removeEmploymentBeneficiaryYesNo"
  ...
  def employmentBeneficiary(id: Int, provisional : Boolean): EmploymentRelatedBeneficiary = EmploymentRelatedBeneficiary(
    name = s"Some Name $id",
    utr = None,
    address = None,
  ...
  val expectedResult: EmploymentRelatedBeneficiary = employmentBeneficiary(2, provisional = true)

  val beneficiaries: List[EmploymentRelatedBeneficiary] = List(
    employmentBeneficiary(1, provisional = false),
    expectedResult,
    employmentBeneficiary(3, provisional = true)
  )

  "RemoveEmploymentBeneficiary Controller" when {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val application = applicationBuilder(userAnswers = Some(userAnswers))
        .overrides(bind[TrustConnector].toInstance(mockConnector)).build()
    ...
    "removing an existing beneficiary" must {
      ...
      "redirect to the next page when valid data is submitted" in {
        ...
        redirectLocation(result).value mustEqual controllers.companyoremploymentrelated.employment.remove.routes.WhenRemovedController.onPageLoad(0).url
