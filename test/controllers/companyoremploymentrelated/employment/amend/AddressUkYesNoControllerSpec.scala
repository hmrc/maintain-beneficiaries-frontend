...
import pages.companyoremploymentrelated.employment.{AddressUkYesNoPage, NamePage}
import play.api.data.Form
import play.api.inject.bind
import play.api.mvc.Call
import play.api.test.FakeRequest
import play.api.test.Helpers._
import views.html.companyoremploymentrelated.employment.amend.AddressUkYesNoView

class AddressUkYesNoControllerSpec extends SpecBase with MockitoSugar {

  private val form: Form[Boolean] = new YesNoFormProvider().withPrefix("employmentBeneficiary.addressUkYesNo")
  private val addressUkYesNoRoute: String = routes.AddressUkYesNoController.onPageLoad().url
  ...
  "AddressUkYesNo Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val answers = baseAnswers.set(AddressUkYesNoPage, true).success.value
    ...
    "redirect to the next page when valid data is submitted" in {
      ...
      val application =
        applicationBuilder(userAnswers = Some(emptyUserAnswers))
          .overrides(
            bind[Navigator].qualifiedWith(classOf[AmendEmploymentRelatedBeneficiary]).toInstance(new FakeNavigator(onwardRoute))
          ).build()
