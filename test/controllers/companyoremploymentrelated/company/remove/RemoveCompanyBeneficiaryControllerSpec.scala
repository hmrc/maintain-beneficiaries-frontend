...
class RemoveCompanyBeneficiaryControllerSpec extends SpecBase with ScalaCheckPropertyChecks with ScalaFutures {
  ...
  "RemoveCompanyBeneficiary Controller" when {
    ...
    "return Not Found and the out of bounds page when getCompanyBeneficiary throws IndexOutOfBoundsException on a GET" in {
    ...
    "return Not Found and the out of bounds page when RemoveCompanyBeneficiaryController throws IndexOutOfBoundsException on a POST" in {
    ...
    "removing an existing beneficiary" must {
      ...
      "redirect to the next page when valid data is submitted" in {
        ...
        redirectLocation(
          result
        ).value mustEqual controllers.companyoremploymentrelated.company.remove.routes.WhenRemovedController
          .onPageLoad(0)
          .url
