...
import config.annotations.AmendCompanyBeneficiary
import forms.YesNoFormProvider
import navigation.{FakeNavigator, Navigator}
import org.scalatestplus.mockito.MockitoSugar
import pages.companyoremploymentrelated.company.{DiscretionYesNoPage, NamePage}
import play.api.data.Form
import play.api.inject.bind
import play.api.mvc.Call
import play.api.test.FakeRequest
import play.api.test.Helpers._
import views.html.companyoremploymentrelated.company.amend.DiscretionYesNoView
...
class DiscretionYesNoControllerSpec extends SpecBase with MockitoSugar {
  ...
  private val discretionYesNoRoute: String = routes.DiscretionYesNoController.onPageLoad().url
    ...
    "redirect to the next page when valid data is submitted" in {
      ...
      val application =
        applicationBuilder(userAnswers = Some(emptyUserAnswers))
          .overrides(
            bind[Navigator].qualifiedWith(classOf[AmendCompanyBeneficiary]).toInstance(new FakeNavigator(onwardRoute))
          ).build()
