...
import pages.companyoremploymentrelated.company.{NamePage, StartDatePage}
...
class StartDateControllerSpec extends SpecBase with MockitoSugar {
  ...
  private val form: Form[LocalDate] = new DateAddedToTrustFormProvider().withPrefixAndTrustStartDate("companyBeneficiary.startDate", date)
  ...
  "NonUkAddress Controller" must {
    ...
    "return a Bad Request and errors when invalid data is submitted" in {
      ...
      val request = FakeRequest(POST, startDateRoute).withFormUrlEncodedBody("value" -> "")
