...
import pages.charityortrust.trust.{AddressUkYesNoPage, NamePage}
import play.api.data.Form
import play.api.inject.bind
import play.api.mvc.Call
import play.api.test.FakeRequest
import play.api.test.Helpers._
import views.html.charityortrust.trust.amend.AddressUkYesNoView

class AddressUkYesNoControllerSpec extends SpecBase with MockitoSugar {

  private val form: Form[Boolean] = new YesNoFormProvider().withPrefix("trustBeneficiary.addressUkYesNo")
    ...
    "redirect to the next page when valid data is submitted" in {
      ...
      val application =
        applicationBuilder(userAnswers = Some(emptyUserAnswers))
          .overrides(
            bind[Navigator].qualifiedWith(classOf[AmendTrustBeneficiary]).toInstance(new FakeNavigator(onwardRoute))
          ).build()
