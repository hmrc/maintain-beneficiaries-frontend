...
class WhenRemovedControllerSpec extends SpecBase with MockitoSugar {
  ...
  private def form = formProvider.withPrefixAndEntityStartDate("trustBeneficiary.whenRemoved", LocalDate.now())

  val validAnswer: LocalDate = LocalDate.now(ZoneOffset.UTC)

  val index = 0

  val description = "Some Name 1"
  ...
  def trustBeneficiary(id: Int): TrustBeneficiary = TrustBeneficiary(
    name = s"Some Name $id",
    utr = None,
    address = None,
    income = None,
