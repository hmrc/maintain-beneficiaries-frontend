...
class RemoveTrustBeneficiaryControllerSpec extends SpecBase with ScalaCheckPropertyChecks with ScalaFutures {
  ...
  def trustBeneficiary(id: Int, provisional: Boolean): TrustBeneficiary = TrustBeneficiary(
    name = s"Some Name $id",
    utr = None,
    address = None,
    income = None,
  ...
  val expectedResult: TrustBeneficiary = trustBeneficiary(2, provisional = true)

  val beneficiaries: List[TrustBeneficiary] = List(
  ...
  "RemoveTrustBeneficiary Controller" when {
    ...
    "removing an existing beneficiary" must {
      ...
      "redirect to the next page when valid data is submitted" in {
        ...
        redirectLocation(result).value mustEqual controllers.charityortrust.trust.remove.routes.WhenRemovedController
          .onPageLoad(0)
          .url
    ...
    "return Not Found and the out of bounds page when getTrustBeneficiary throws IndexOutOfBoundsException on a GET" in {
    ...
    "return Not Found and the out of bounds page when RemoveTrustBeneficiaryController throws IndexOutOfBoundsException on a POST" in {
