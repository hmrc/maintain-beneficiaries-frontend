...
class AddABeneficiaryControllerSpec extends SpecBase with ScalaFutures {
  ...
  val beneficiaryRows = List(
    ...
    AddRow("Trust Beneficiary Name", typeLabel = "Named trust", "Change details", Some(controllers.charityortrust.trust.add.routes.NameController.onPageLoad().url), "Remove", Some(controllers.charityortrust.trust.remove.routes.RemoveTrustBeneficiaryController.onPageLoad(0).url)),
