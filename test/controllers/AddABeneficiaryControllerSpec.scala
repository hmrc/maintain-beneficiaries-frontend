...
class AddABeneficiaryControllerSpec extends SpecBase with ScalaFutures {
  ...
  val beneficiaryRows = List(
    ...
    AddRow("Humanitarian Endeavours Ltd", typeLabel = "Named charity", "Change details", Some(controllers.charityortrust.amend.charity.routes.CheckDetailsController.extractAndRender(0).url), "Remove", Some(controllers.charityortrust.remove.charity.routes.RemoveCharityBeneficiaryController.onPageLoad(0).url)),
