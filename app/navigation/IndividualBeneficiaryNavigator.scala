...
class IndividualBeneficiaryNavigator @Inject() () extends Navigator {
  ...
  private def simpleNavigation(mode: Mode): PartialFunction[Page, UserAnswers => Call] = {
    ...
    case PassportDetailsPage | IdCardDetailsPage | PassportOrIdCardDetailsPage =>
      ua => navigateAwayFromPassportIdCardCombined(mode)
  ...
  private def yesNoNavigation(mode: Mode): PartialFunction[Page, UserAnswers => Call] = {
    ...
    case IncomeDiscretionYesNoPage        =>
      ua =>
        yesNoNav(
          ua = ua,
    ...
    case CountryOfNationalityYesNoPage    =>
      ua =>
        yesNoNav(
          ua = ua,
    ...
    case CountryOfNationalityUkYesNoPage  =>
      ua =>
        yesNoNav(
          ua = ua,
    ...
    case PassportDetailsYesNoPage         =>
      ua =>
        yesNoNav(
          ua,
          PassportDetailsYesNoPage,
          rts.PassportDetailsController.onPageLoad(mode),
          rts.IdCardDetailsYesNoController.onPageLoad(mode)
        )
    case IdCardDetailsYesNoPage           =>
      ua =>
        yesNoNav(
          ua = ua,
    ...
    case PassportOrIdCardDetailsYesNoPage =>
      ua =>
        if (mode == NormalMode) {
          yesNoNav(
            ua,
            PassportOrIdCardDetailsYesNoPage,
            rts.PassportOrIdCardDetailsController.onPageLoad(mode),
  ...
  private def trustTypeNavigation(mode: Mode): PartialFunction[Page, UserAnswers => Call] = { case NamePage =>
    ua => trustTypeNav(mode, ua)
  }
  ...
  private def modeNavigation(mode: Mode): PartialFunction[Page, UserAnswers => Call] =
    mode match {
      case NormalMode => { case VPE1FormYesNoPage =>
        _ => addRts.StartDateController.onPageLoad()
      }
      case CheckMode  => { case VPE1FormYesNoPage =>
        ua => modeNav(ua)
      }
    }
