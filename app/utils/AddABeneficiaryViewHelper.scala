...
class AddABeneficiaryViewHelper {
  ...
  def rows(
    ...
    trustType: Option[TypeOfTrust] = None
  )(implicit messages: Messages): AddToRows = {

    implicit class BeneficiaryRows[T <: Beneficiary](beneficiaries: List[T]) {
      def zipThenGroupThenMap(row: (T, Int) => AddRow, isComplete: Boolean): List[AddRow] = beneficiaries.zipWithIndex
    ...
    def individualBeneficiaryRow(beneficiary: IndividualBeneficiary, index: Int): AddRow =
      AddRow(
        ...
        removeUrl = Some(
          controllers.individualbeneficiary.remove.routes.RemoveIndividualBeneficiaryController.onPageLoad(index).url
        )
      )
    ...
    def classOfBeneficiaryRow(beneficiary: ClassOfBeneficiary, index: Int): AddRow =
      AddRow(
        ...
        removeUrl =
          Some(controllers.classofbeneficiary.remove.routes.RemoveClassOfBeneficiaryController.onPageLoad(index).url)
      )
    ...
    def trustBeneficiaryRow(beneficiary: TrustBeneficiary, index: Int): AddRow =
      AddRow(
        ...
        removeUrl =
          Some(controllers.charityortrust.trust.remove.routes.RemoveTrustBeneficiaryController.onPageLoad(index).url)
      )
    ...
    def charityBeneficiaryRow(beneficiary: CharityBeneficiary, index: Int): AddRow =
      AddRow(
        ...
        removeUrl = Some(
          controllers.charityortrust.charity.remove.routes.RemoveCharityBeneficiaryController.onPageLoad(index).url
        )
      )
