...
import pages.other.CountryOfResidencePage
...
class CountryOfResidenceController @Inject()(
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = standardActionSets.verifiedForUtr.andThen(nameAction) {
    implicit request =>
      ...
      Ok(view(preparedForm, mode, request.description, countryOptions.options()))
  }
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = standardActionSets.verifiedForUtr.andThen(nameAction).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        formWithErrors =>
          Future.successful(BadRequest(view(formWithErrors, mode, request.description, countryOptions.options()))),
