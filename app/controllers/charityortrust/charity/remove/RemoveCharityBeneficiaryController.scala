      ...
      trustService.getCharityBeneficiary(request.userAnswers.utr, index).map {
        ...
        case _ =>
          logger.error(s"[Session ID: ${utils.Session.id(hc)}][UTR: ${request.userAnswers.utr}]" +
            s" error getting charity beneficiary $index from trusts service")
        ...
        value => {
          ...
          if (value) {
            ...
            trustService.getCharityBeneficiary(request.userAnswers.utr, index).flatMap {
              ...
              case _ =>
                logger.error(s"[Session ID: ${utils.Session.id(hc)}][UTR: ${request.userAnswers.utr}]" +
                  s" error removing a charity beneficiary as could not get beneficiary $index from trusts service")
