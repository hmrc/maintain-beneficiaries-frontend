...
class WhenRemovedController @Inject() (
  ...
  def onPageLoad(index: Int): Action[AnyContent] = standardActionSets.verifiedForUtr.async { implicit request =>
    trust.getCharityBeneficiary(request.userAnswers.identifier, index).map { beneficiary =>
  ...
  def onSubmit(index: Int): Action[AnyContent] = standardActionSets.verifiedForUtr.async { implicit request =>
    trust.getCharityBeneficiary(request.userAnswers.identifier, index).flatMap { beneficiary =>
      val form = formProvider.withPrefixAndEntityStartDate("charityBeneficiary.whenRemoved", beneficiary.entityStart)
      form
        .bindFromRequest()
        .fold(
          ...
          value =>
            trustService
              .removeBeneficiary(
                ...
                RemoveBeneficiary(BeneficiaryType.CharityBeneficiary, index, value)
              )
              .map { _ =>
                logger.info(
                  s"[Session ID: ${utils.Session.id(hc)}][UTR: ${request.userAnswers.identifier}]" +
