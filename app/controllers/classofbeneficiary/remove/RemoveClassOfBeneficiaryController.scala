      ...
      .fold(
        ...
        value =>
          if (value) {
            ...
            trustService.getUnidentifiedBeneficiary(request.userAnswers.identifier, index).flatMap { beneficiary =>
              if (beneficiary.provisional) {
                trustService
                  .removeBeneficiary(
                    request.userAnswers.identifier,
                    RemoveBeneficiary(BeneficiaryType.ClassOfBeneficiary, index)
                  )
                  .map { _ =>
                    logger.info(
