...
import pages.companyoremploymentrelated.employment.CountryOfResidencePage
...
class CountryOfResidenceController @Inject() (
  ...
  private val form: Form[String] = formProvider.withPrefix("employmentBeneficiary.countryOfResidence")
    ...
    standardActionSets.verifiedForUtr.andThen(nameAction).async { implicit request =>
      form
        ...
        .fold(
          formWithErrors =>
            Future
              .successful(BadRequest(view(formWithErrors, mode, request.beneficiaryName, countryOptions.options()))),
          value =>
            for {
