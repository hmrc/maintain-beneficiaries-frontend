...
class RemoveEmploymentBeneficiaryController @Inject() (
  ...
  def onPageLoad(index: Int): Action[AnyContent] = standardActionSets.identifiedUserWithData.async { implicit request =>
    ...
    trustService.getEmploymentBeneficiary(request.userAnswers.identifier, index).map { beneficiary =>
      ...
      .fold(
        ...
        value =>
          if (value) {
            ...
            trustService.getEmploymentBeneficiary(request.userAnswers.identifier, index).flatMap { beneficiary =>
              if (beneficiary.provisional) {
                trustService
                  .removeBeneficiary(
                    request.userAnswers.identifier,
                    RemoveBeneficiary(BeneficiaryType.EmploymentRelatedBeneficiary, index)
                  )
                  .map { _ =>
                    logger.info(
                      s"[Session ID: ${utils.Session.id(hc)}][UTR: ${request.userAnswers.identifier}]" +
                ...
                for {
                  ...
                  controllers.companyoremploymentrelated.employment.remove.routes.WhenRemovedController
                    .onPageLoad(index)
                    .url
                )
              }
