    ...
    form
      ...
      .fold(
        (formWithErrors: Form[_]) =>
          trustService.getIndividualBeneficiary(request.userAnswers.identifier, index).map { beneficiary =>
            BadRequest(view(formWithErrors, index, beneficiary.name.displayName))
          },
        value =>
          if (value) {

            trustService.getIndividualBeneficiary(request.userAnswers.identifier, index).flatMap { beneficiary =>
              if (beneficiary.provisional) {
                trustService
                  .removeBeneficiary(
                    request.userAnswers.identifier,
                    RemoveBeneficiary(BeneficiaryType.IndividualBeneficiary, index)
                  )
                  .map { _ =>
                    logger.info(
                ...
                for {
                  ...
                  controllers.individualbeneficiary.remove.routes.WhenRemovedController.onPageLoad(index)
                )
              }
            }
          } else {
